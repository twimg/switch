...
.rank-table th, .rank-table td {
  padding:4px 8px; font-size:14px; color:#fff;
  border-bottom:1px solid #314265;
}
.rank-table thead th { background:rgba(32,44,70,0.8); }
</style>
...
# --- 画像プール ---
face_imgs = [f"https://randomuser.me/api/portraits/men/{i}.jpg" for i in range(10,90)]
...
# --- タブ ---
tabs = st.tabs(["Senior","Youth","Match","Scout","Rankings","Standings","Save"])
...
with tabs[2]:
    ...
    formation = st.selectbox("Formation",["4-4-2","4-3-3","3-5-2"], key="form")
    ...
    if st.session_state.starters:
        ...
        for p in st.session_state.starters:
            st.write(f"- {p}")
...
with tabs[3]:
    ...
    # 候補表示
    for df_cand, key_pref, label in [
        (st.session_state.scout_s, "ss", "Senior Candidates"),
        (st.session_state.scout_y, "sy", "Youth Candidates")
    ]:
...
# ==== 5. Rankings ====
with tabs[4]:
    st.markdown('<div class="stage-label">Top 5 Rankings</div>', unsafe_allow_html=True)
    # ダミーデータ生成
    all_players = pd.concat([st.session_state.senior, st.session_state.youth], ignore_index=True)
    all_players["Goals"] = all_players["OVR"]//15 + np.random.randint(0,5,len(all_players))
    all_players["Assists"] = all_players["OVR"]//20 + np.random.randint(0,3,len(all_players))
    all_players["Rating"] = np.round(all_players["OVR"]/10 + np.random.random(len(all_players))*2,1)
    all_players["MVP"] = np.random.randint(0,3,len(all_players))
    top_goals = all_players.nlargest(5,"Goals")[["Name","Club","Goals"]]
    top_assists = all_players.nlargest(5,"Assists")[["Name","Club","Assists"]]
    top_rating = all_players.nlargest(5,"Rating")[["Name","Club","Rating"]]
    top_mvp = all_players.nlargest(5,"MVP")[["Name","Club","MVP"]]
    def render_rank(df, title):
        st.markdown(f"**#{title}**")
        st.table(df.style.set_properties(**{"color":"white","background":"rgba(32,44,70,0.7)"}))
    c1,c2,c3,c4 = st.columns(4)
    with c1: render_rank(top_goals,"Goals")
    with c2: render_rank(top_assists,"Assists")
    with c3: render_rank(top_rating,"Rating")
    with c4: render_rank(top_mvp,"MVP")

# ==== 6. Standings ====
with tabs[5]:
    st.markdown('<div class="stage-label">Standings</div>', unsafe_allow_html=True)
    dfst = st.session_state.stand.copy()
...
st.caption("2025年版：全機能統合・レーダー表示・ランキング・完全版")
