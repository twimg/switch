...
LEAGUES = {
    'イングランド': {
        '1部': [MY_CLUB,"Midtown United","Eastport Rovers","Kingsbridge Athletic","Westhaven City","Southvale Town","Northgate FC","Oakwood Albion"],
...
NAME_POOLS = {
    ...
    'ITA': {
        'given': ["Lorenzo","Alessandro","Francesco","Mattia","Leonardo","Riccardo","Gabriele","Niccolò","Tommaso","Andrea","Marco","Matteo","Fabio","Emanuele","Valerio","Daniele","Federico","Simone","Alberto","Vincenzo","Stefano","Davide","Giovanni","Fabiano","Luca","Antonio","Paolo","Maurizio","Raffaele","Jonathan"],
    ...
    'ESP': {
        'given': ["Hugo","Martín","Lucas","Mateo","Iker","Diego","Álvaro","Pablo","Adrián","Sergio","Joaquín","Ángel","David","Rubén","Martí","Óscar","Víctor","Miguel","Enzo","Álex","Bruno","Mario","Oliver","Juan","José","Raúl","Isco","Pedro","Nacho","Saúl"],
    ...
    'FRA': {
        'given': ["Lucas","Gabriel","Léo","Raphaël","Arthur","Louis","Hugo","Jules","Adam","Nathan","Ethan","Thomas","Clément","Théo","Mathis","Noah","Maxime","Paul","Alexis","Victor","Martin","Gabin","Quentin","Guillaume","Baptiste","Maxence","Romain","Antoine","Mathieu","Robin"],
    ...
    'BRA': {
        'given': ["Pedro","Lucas","Guilherme","Mateus","Gabriel","Rafael","Bruno","Thiago","Felipe","Diego","Vinícius","João","Carlos","Ricardo","Eduardo","Fernando","Rodrigo","Paulo","Leandro","André","Vitor","Marcelo","Roberto","Caio","Renato","Igor","Luan","Fábio","Jonas","Samuel"],
...
with tabs[6]:
    if not ses.intl_tournament:
        parts = []
        for reg in regions:
            top2 = ses.standings[reg]['1部'].nlargest(2,'Pts')['Club'].tolist()
            parts.extend(top2)
    ...
    for idx, m in enumerate(ses.intl_tournament['results']):
        line = f"【R{idx+1}】 {m[0]} {m[1]}-{m[3]} {m[2]} → {m[4]}"
        if MY_CLUB in line:
    ...
    if len(ses.intl_tournament['clubs']) == 1:
        champ_line = f"優勝: {ses.intl_tournament['clubs'][0]}"
...
with tabs[9]:
    ...
    if typ == '国籍別得点':
        ...
        st.dataframe(df_nat.rename(columns={'Nat':'国籍','Goals':'総ゴール'}),
                     use_container_width=True)
    elif typ == '国籍別平均OVR':
