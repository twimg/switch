...
.player-card + .player-card { border-top:0.6px solid #ccc; padding-top:12px; }
.player-card img {
...
def gen_players(n, youth=False):
    ...
    for _ in range(n):
        ...
        img_i = random.randrange(len(face_imgs))
        lst.append({
            "Name":     name,
            "Nat":      nat,
...
with tabs[0]:
    ...
    kw = st.text_input("Search Name (Senior)")
    df1 = st.session_state.senior.copy()
    if kw:
        df1 = df1[df1["Name"].str.contains(kw, case=False)]
    ...
    for i,r in df1.iterrows():
        ...
        with c1:
            st.image(get_img(r["img_idx"]), width=48)
        with c2:
            st.markdown(f"**{r['Name']}**")
            st.markdown(f"{NATIONS[r['Nat']]} ｜ {r['Pos']} ｜ {r['Age']}")
            st.markdown(f"OVR: {r['OVR']}")
        ...
        if st.session_state.detail == key:
            abil = [r[l] for l in labels] + [r[labels[0]]]
            ...
            st.pyplot(fig)
            stats = "".join(
                f"<span style='color:{'#20e660' if r[l]>=90 else '#ffe600' if r[l]>=75 else '#1aacef'}'>{l}:{r[l]}</span><br>"
                for l in labels
            )
...
with tabs[1]:
    ...
    kw2 = st.text_input("Search Name (Youth)")
    df2 = st.session_state.youth.copy()
    if kw2:
        df2 = df2[df2["Name"].str.contains(kw2, case=False)]
    if df2.empty:
...
with tabs[2]:
    ...
    if st.session_state.starters:
        ...
        ax.text(5,1,names[idx],ha='center',va='center',color='yellow'); idx+=1
        for x in def4:
            ax.text(x,4,names[idx],ha='center',va='center',color='white'); idx+=1
        for x in mid:
            ax.text(x,8,names[idx],ha='center',va='center',color='white'); idx+=1
        for x in fw:
            ax.text(x,12,names[idx],ha='center',va='center',color='white'); idx+=1
        st.pyplot(fig)
...
with tabs[3]:
    ...
    if not st.session_state.scout_s.empty:
        ...
        for i,row in st.session_state.scout_s.iterrows():
            ...
            with col1:
                st.image(get_img(row["img_idx"]), width=48)
            with col2:
...
st.caption("2025年版：完全統合・モバイル対応・詳細トグル・国籍絵文字・ソート検索対応")
