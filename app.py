...
with tabs[0]:
    ...
    for i,row in df1.iterrows():
        ...
        with cols[1]:
            st.write(f"**{row['Name']}** ({row['Age']}y)  {row['Nat']}｜{row['Pos']}")
...
# ==== 3. Match ====
with tabs[2]:
    st.markdown(f'<div class="stage-label">Match Simulation ‒ Week {st.session_state.current_week}</div>', unsafe_allow_html=True)
...
st.caption("2025年版：Week管理＆Transfer Log統合版 完成仮版")```

**変更点まとめ**  
1. **Week管理**：`current_week` をセッションで管理、Kickoff後に自動進行  
2. **Transfer Log**：スカウト成功時に `transfer_log` へ追加し、新タブで履歴を表示  
3. **タブ構成**：`Transfers` タブを追加、`Save` は最後に配置  
4. **細かなUI**：選手間の区切り線、ボタン色・テーブル透過・白文字など

これで“仮完成品”に①入退団ログと③節管理を統合した形になります。  
次のステップ（成長・負傷や代理人交渉など）も、この土台に追加しやすくなっています。
