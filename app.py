...
.stDataFrame {background:#253457cc!important; color:#fff!important;}
</style>
...
with tabs[2]:
    ...
    if st.button("Kickoff!"):
        ...
        goals_you = max(0, int(np.random.normal((atk-60)/8,1)))
        goals_op = max(0, int(np.random.normal((opp_atk-60)/8,1)))
        result = "Win" if goals_you>goals_op else "Lose" if goals_you<goals_op else "Draw"
        mvp = ours.loc[ours['OVR'].idxmax(), 'Name']
        st.info(f"**Result: {result} ({goals_you}-{goals_op})**")
        st.success(f"Goals: You {goals_you} - Opp {goals_op} | MVP: {mvp}")
        # 順位表更新
...
with tabs[4]:
    ...
    dfst = st.session_state.stand
    st.dataframe(
        dfst.style.set_properties(**{"background-color":"#202c46","color":"#fff"}).hide_index(),
        height=350
    )
...
st.caption("2025年最新版：モバイル完全対応・全機能統合版（詳細／試合／Scout／Standings）")
