...
.player-card + .player-card {
  border-top:0.3px solid #ccc; padding-top:6px;
}
.player-card img {
...
with tabs[2]:
    ...
    if st.session_state.starters:
        def surname(name): return name.split()[-1]
        coords = {
        ...
        ax.text(5,1, surname(names[idx]), ha='center', va='center', color='yellow'); idx+=1
        for x in def4:
            ax.text(x,4, surname(names[idx]), ha='center', va='center', color='white'); idx+=1
        for x in mid:
            ax.text(x,8, surname(names[idx]), ha='center', va='center', color='white'); idx+=1
        for x in fw:
            ax.text(x,12, surname(names[idx]), ha='center', va='center', color='white'); idx+=1
        st.pyplot(fig)
